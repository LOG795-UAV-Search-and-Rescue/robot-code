...
class ReadLine:
	def __init__(self, s):
		...
		try:
			...
			print(f"[ReadLine.__init__] error: {e}")
			self.lidar_ser = None
	...
	def parse_lidar_frame(self, data):
		...
		for i in range(0, self.ANGLE_PER_FRAME):
			...
			if(distance > 2000 or distance < 600):
				continue
...
class BaseController:
	...
	def lights_ctrl(self, pwm_a, pwm_b):
		data = {"T":132,"IO4":pwm_a,"IO5":pwm_b}
		self.send_command(data)
		self.base_light_status = pwm_a
		self.head_light_status = pwm_b
...
if __name__ == '__main__':
	...
	base.base_lights_ctrl(255, 0)

	while True:
		try:
			base.gimbal_ctrl(-90, 0, 0, 0)
			time.sleep(2)
			base.gimbal_ctrl(90, 60, 0, 0)
			time.sleep(2)
		except Exception as e:
			print(f"[base_ctrl] error: {e}")
			break
