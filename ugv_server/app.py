...
def get_kpi():
    nb_points = int(request.args.get('n', 0))
    kpi_data = map_ctrl.kpi[-nb_points:] if len(map_ctrl.kpi) >= nb_points and nb_points > 0 else map_ctrl.kpi
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    cmd_triggered = False
    quality_min = 40.0  # minimum acceptable VIO quality
    drone_x = 0.0
    drone_y = 0.0
    last_good_x = 0.0
    last_good_y = 0.0
