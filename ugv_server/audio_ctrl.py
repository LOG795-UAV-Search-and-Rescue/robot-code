...
def play_random_audio(input_dirname, force_flag):
	...
	if play_audio_event.is_set() and not force_flag:
		return
	audio_files = [f for f in os.listdir(current_path + SOUND_DIR + input_dirname) if f.endswith((".mp3", ".wav"))]
	audio_file = random.choice(audio_files)
	play_audio_event.set()
	audio_thread = threading.Thread(target=play_audio, args=(current_path + SOUND_DIR + input_dirname + "/" + audio_file,))
	audio_thread.start()
...
def play_file(audio_file):
	if not usb_connected:
		return
	audio_file = current_path + SOUND_DIR + audio_file
